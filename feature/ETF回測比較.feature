...
Feature: ETF 歷史回測比較
  ...
  @Logic @ModeCoreDifference
  Scenario: 兩種模式的核心差異
    """
    ETF 比較模式與投資組合模式的回測結果應該有本質區別：
    - ETF 比較模式：各 ETF 獨立計算，顯示每檔 ETF 的個別績效
    - 投資組合模式：將多個 ETF 加權組合，計算組合整體的績效與風險
    """
    Given 使用者選擇 VTI (60%) 和 BND (40%)
    
    When 在「ETF 比較」模式下回測
    Then 結果應該顯示兩張獨立卡片:
      | ETF   | 最終價值      | 報酬率   | 說明               |
      | VTI   | $156,789      | +156%    | VTI 個別績效       |
      | BND   | $42,345       | +28%     | BND 個別績效       |
    And 圖表顯示兩條獨立的走勢線
    
    When 在「投資組合」模式下回測
    Then 結果應該顯示單一組合卡片:
      | 組合       | 配置            | 最終價值      | 報酬率   | 說明                      |
      | 我的組合   | VTI 60%+BND 40% | $105,678      | +105.7%  | 加權組合的整體績效         |
    And 圖表顯示一條組合的加權走勢線
    And 額外顯示進階風險指標 (標準差、Beta、夏普比率等)
    And 顯示各 ETF 對組合報酬的貢獻度明細
  ...
  Scenario: 投資組合回測結果
    ...
    Then 應該顯示組合績效摘要:
      ...
      | 總投入本金         | NT$ 1,200,000  | NT$ 1,200,000   |
      | 總報酬率           | +169.5%        | +139.7%         |
      | 年化報酬率 (CAGR)  | 8.5%           | 7.2%            |
    And 顯示各組合的報酬/風險效率比較

  @UI @PortfolioRiskMetrics
  Scenario: 投資組合進階風險指標
    Given 完成投資組合回測
    Then 應該顯示進階風險指標:
      | 指標                              | 說明                                          | 我的組合範例 | 經典 60/40 |
      | 標準差 (Standard Deviation)       | 衡量報酬率的總波動性，數值越高代表風險越大     | 15.2%        | 10.8%      |
      | 年化波動率 (Annualized Volatility) | 標準差的年化值，方便與其他資產比較             | 14.8%        | 10.5%      |
      | 最大回撤 (Maximum Drawdown)       | 從最高點到最低點的最大虧損幅度                 | -28.5%       | -22.1%     |
      | Beta 值 (Beta Coefficient)        | 相對於大盤 (SPY) 的敏感度，1 代表與大盤同步    | 0.85         | 0.58       |
      | 夏普比率 (Sharpe Ratio)           | 每承擔 1 單位風險所獲得的超額報酬              | 0.69         | 0.73       |
      | 索提諾比率 (Sortino Ratio)        | 只考慮下跌風險的報酬/風險比，比夏普更精確      | 0.92         | 0.98       |
      | 崔納值 (Treynor Ratio)            | 每承擔 1 單位系統性風險 (Beta) 的超額報酬      | 0.10         | 0.12       |
    And 每個指標應該有 tooltip 說明其意義
    And 顯示「較佳」的指標應以綠色標示

  @UI @RiskMetricsComparison
  Scenario: 風險指標比較雷達圖
    Given 完成多個投資組合回測
    Then 應該顯示風險指標雷達圖:
      | 雷達圖軸線   | 說明                                |
      | 報酬率       | 年化報酬率，數值越高越好             |
      | 風險控制     | 反向標準差，數值越高代表風險越低      |
      | 抗跌能力     | 反向最大回撤，數值越高代表抗跌越強    |
      | 風險調整報酬 | 夏普比率標準化，數值越高越好          |
      | 下跌保護     | 索提諾比率標準化，數值越高越好        |
    And 可視覺化比較不同組合的風險報酬特性

  @UI @PortfolioChart
  Scenario: 投資組合績效圖表
    Given 完成投資組合回測 (VTI 60% + BND 40%)
    Then 應該顯示累積報酬走勢圖:
      | 線條           | 說明                                      |
      | 組合走勢 (粗)  | 加權組合的整體累積報酬，以較粗線條顯示     |
      | VTI 走勢       | VTI 的個別累積報酬，以淺色細線顯示         |
      | BND 走勢       | BND 的個別累積報酬，以淺色細線顯示         |
    And 可切換顯示/隱藏個別 ETF 走勢線
    And 懸停時同時顯示組合與各 ETF 當時的報酬率

  @UI @PortfolioVsETFComparison
  Scenario: 投資組合與個別 ETF 指標比較表
    Given 完成投資組合回測 (VTI 60% + BND 40%)
    Then 應該顯示指標比較表:
      | 指標           | 組合 (加權) | VTI (個別) | BND (個別) | 組合優勢說明                    |
      | 年化報酬率     | 8.5%        | 10.2%      | 4.1%       | 介於兩者之間                    |
      | 標準差         | 10.8%       | 15.6%      | 5.2%       | ✅ 比 VTI 低 31% (分散風險)     |
      | 最大回撤       | -22.1%      | -35.5%     | -12.3%     | ✅ 比 VTI 低 38% (抗跌)         |
      | 夏普比率       | 0.73        | 0.62       | 0.58       | ✅ 風險調整報酬最佳             |
      | 索提諾比率     | 0.98        | 0.78       | 0.72       | ✅ 下跌風險控制最佳             |
      | Beta           | 0.58        | 1.02       | 0.15       | ✅ 對大盤敏感度適中             |
    And 每個「組合優勢」都應以綠色標示
    And 此表格幫助使用者理解「分散投資」的實際效益

  @UI @PortfolioBreakdown  
  Scenario: 組合成分績效明細
    Given 完成投資組合回測
    Then 應該顯示各成分 ETF 的績效明細:
      | ETF   | 配置比例 | 個別報酬率 | 對組合貢獻 | 個別風險 | 對組合風險貢獻 |
      | VTI   | 60%      | +156%      | +93.6%     | 15.6%    | 9.4%           |
      | BND   | 40%      | +28%       | +11.2%     | 5.2%     | 2.1%           |
    And 顯示組合配置圓餅圖
    And 顯示各 ETF 貢獻度長條圖
    And 可展開查看各 ETF 的詳細走勢圖與年度報酬
